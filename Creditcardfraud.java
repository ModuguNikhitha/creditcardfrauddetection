...
public class Creditcardfraud extends Applet implements ActionListener {
    ...
    public void init() {
        ...
        add(l3);
        add(con);
//        register to receive action events browse.addActionListener(this);
...
// System.out.println(" 1");
                    ...
                    for (int i = 0; i <= 20; i++) {
                        ...
                        if (i == 0) {
                            db.append("\n");
                            db.append("______\t____\t______\t__\t______\t__\t_____\t___\t___\t____\t___\t__ __");
                            db.append("\n");
                        }
                        db.append("\n");
                        System.out.println(" ");
            ...
            ssdi_project.Detection dt = new ssdi_project.Detection();
            ssdi_project.Evaluate ev = new ssdi_project.Evaluate();
            ssdi_project.NextGen ng = new ssdi_project.NextGen();
            /* CC usage Fequency */
            int l = 0, m = 0;
            ...
            for (int i = 1; i <= 20; i++) {
                ...
                if (res[0] >= 1) {
                    fre[l][m] = Float.valueOf(data[i][0]);
                    m++;
                    fre[l][m] = res[1];
            ...
            for (int i = 1; i <= 20; i++) {
                res = dt.ccloc(data[i]);
                if (res[0] >= 1) {
                    loc[l][m] = Float.valueOf(data[i][0]);
                    m++;
                    loc[l][m] = res[1];
...
// float[][] finalresult = dt.organize(fre,loc,od,bb,ds);
            ...
            for (int q = 1; q <= 20; q++) {
                ...
                System.out.println(" \n");
                System.out.println(" Current Population - Generation - " + q);
            ...
            System.out.println("--------------------------------------------- ");
            con.append(" Critical Fraud Detected: ");
            con.append("\n");
            con.append("------------------------------------- ");
